$(document).ready(function() {
	// Variable to track whether the user confirmed the action
	var confirmed = false;

	var currentRole = $("#currentUserRole").text();

	//var currentRole = $("#showStatusButton").text();
	...
	var table = $('#userTable').DataTable({
		...
		columns: [
...

			{
				...
				render: function(data, type, row) {
					var statusText = data ? "Inactive" : "Active";
					var btnClass = data ? "inactive-btn" : "active-btn";

					if (currentRole === "Admin") {
						if (row.role === "Junior-HR" || row.role === "Senior-HR" || row.role === "Interviewer" || row.role === "Department-head") {
							return '<div class="btn-group" role="group">' +
								'<button type="button" class="btn ' + btnClass + ' statusButton" data-user-id="' + row.id + '" data-user-status="' + data + '" id="statusButton">' + statusText + '</button>' + '</div>';
						...
						if (row.role !== "Default-Admin") {
							...
							return '<div class="btn-group" role="group">' +
								'<button type="button" class="btn ' + btnClass + ' statusButton" data-user-id="' + row.id + '" data-user-status="' + data + '" id="statusButton">' + statusText + '</button>' +
								'</div>';
					...
					if (currentRole === "Admin") {
						if (row.role === "Junior-HR" || row.role === "Senior-HR" || row.role === "Interviewer" || row.role === "Department-head") {
							...
							if (row.role === "Junior-HR") {
								...
								permissionButton = '<button type="button" class="btn permission-btn" data-user-id="' + row.id + '" data-user-role="Senior-HR" id="permissionBtn"><i class="bi bi-arrow-up-circle-fill"></i></button>';
							} else if (row.role === "Senior-HR") {
								permissionButton = '<button type="button" class="btn permission-btn" data-user-id="' + row.id + '" data-user-role="Admin" id="permissionBtn"><i class="bi bi-arrow-up-circle-fill"></i></button>';
							}
							return '<div class="btn-group" role="group">' +
	...
	function showModal(userId, userRole) {
		// Update the content of the modal dynamically based on the user role
	...
	$('#userTable').on('click', '.statusButton', function() {
		...
		var statusToggle = button.hasClass('inactive-btn');
		...
		confirmationModal.modal('show').one('click', '#confirmButton', function() {
			...
			if (invertedStatusToggle) {
				button.removeClass('inactive-btn').addClass('active-btn').text('Active');
			} else {
				button.removeClass('active-btn').addClass('inactive-btn').text('Inactive');
			}
...
$(document).ready(function() {
	...
	function checkDuplicateEmail(email) {
		$.ajax({
			...
			success: function(response) {
				if (response) {
					displayErrorMessage('#email-error', email + ' already exists');
				} else {
	...
	function validateForm() {
		...
		var submitButton = $('#addUserButton'); // Replace 'submitButton' with the ID or selector of your actual submit button
