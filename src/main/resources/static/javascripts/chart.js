...
function updateBarChart1() {
    // get the selected values and change them to int
...
function updateMonthSelectDisplay2() {
    ...
    if (selectedYear === '0') {
        // hide the month when 'All' is selected for year
        monthSelectDiv.style.display = 'none';
...
function updateLineChart() {
    ...
    fetch(`/hr/position-demand/${selectedStartYear}/${selectedEndYear}`)
        ...
        .then(data => {
            ...
            const labels = [...yearsSet].sort((a, b) => a - b); // sort years for ascending order
            console.log("Labels:", labels);

            // group data by position name and year
...
function updateBarChart3(departmentId, isUrgent) {
    // get selected values and change them to int
...
function createDoughnutChart(data, vacancyName) {
    ...
    // set the title based on vacancyName and positionSelectOption2 value
    let title;
